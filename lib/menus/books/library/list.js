...
async function response(link, choices, dependencies) {
  const links = parseLinks(link);

  return choose(links, choices, dependencies);
}

async function getAPI(goBack, dependencies = {}) {
  ...
  return inquirer.prompt({
    ...
    .then(({ volume }) => {
      ...
      return request.then(({ body, headers: { link } }) => {
        ...
        return response(link, choices, dependencies);
      });
    });
}
...
async function run(goBack, dependencies = {}, url) {
  ...
  return getAPI(goBack, dependencies)
    .then(({ listBooks, links }) => {
      return inquirer.prompt(listBooks)
        .then(({ book }) => {
