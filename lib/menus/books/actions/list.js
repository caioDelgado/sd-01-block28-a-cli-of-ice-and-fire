...
const caseObjIsTrue = async (goBack, dependencies = {}, valuebyFetch, nextOrPrev) => {
  const line = new inquirer.Separator();
  const choices = [];
  const link = parseLinks(nextOrPrev);
  choices.push(line);
  await valuebyFetch.map((book) =>  choices.push({ name: book.name, value: book }));
