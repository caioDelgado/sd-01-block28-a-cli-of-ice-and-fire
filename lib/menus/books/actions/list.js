...
async function teste(goBack, dependencies = {}, run, choices, links, url) {
...
async function ready(goBack, dependencies, url, run) {
  ...
  inquirer.prompt({
    ...
    .then(({ searchBooks }) => {
      ...
      request.then(({ body, headers: { link } }) => {
        ...
        teste(goBack, dependencies, run, choices, links, url);
      });
    });
}

async function run(goBack, dependencies = {}, url) {
  return ready(goBack, dependencies, url, run);
}
