...
function generateFirstInquirer(goBack, endpoint, links, choices) {
  return inquirer
    .prompt({
      type: 'list',
    ...
    .then(({ book }) => {
      ...
      console.log(book);
      return generateSecondInquirer(goBack, endpoint);
    });
}

function generateSecondInquirer(goBack, endpoint) {
  return inquirer
    .prompt({
      type: 'list',
...
async function search(goBack, endpoint, url) {
  ...
  choices.push(new inquirer.Separator());
  return generateFirstInquirer(goBack, endpoint, links, choices);
}
