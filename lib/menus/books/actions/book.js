...
const addLinks = (choices, link, { inquirer }) => {
  ...
  options.push({ name: 'Voltar para o menu de Livros', value: 'back' });
...
async function run(goBack, dependencies = {}, url, nameBook) {
  try {
    const { inquirer, superagent = superagentModule } = dependencies;

    const { body, headers: { link } } = await superagent.get(url || `https://www.anapioficeandfire.com/api/books?name=${nameBook}`);
