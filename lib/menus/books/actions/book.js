...
const { linkAndUrl } = require('../../LinkAndUrl.js');

const filterBook = (book) => (
  Object.entries(book)
    .filter((obj) => {
      return (obj[0] !== 'characters' && obj[0] !== 'povCharacters')
    }).reduce((acc, obj) => ({ ...acc, [obj[0]]: obj[1] }), {})
);
...
async function run(goBack, dependencies = {}, url) {
  try {
    ...
    return inquirer.prompt({
      ...
      .then(answers => handleChoice(answers, goBack, dependencies, linkAndUrl(url, link, 'Deseja consultar outro livro?'), run));
  } catch (err) {
