...
// const charactersFixture = require('../../../../test/fixtures/books');
...
//     beforeAll(async () => {
//       const inquirer = {
//         prompt: jest.fn().mockResolvedValue({ selectChoice: {} }),
...
//       expect(superagent.get).toBeCalledWith('https://www.anapioficeandfire.com/api/characters');
//     });
...
//   describe('details', () => {
//     const superagent = getSuperagentMock(charactersFixture.responses.hasNext);
//     const selectChoice = charactersFixture.responses.hasNext.body[0];
//     const mockPrompt = jest.fn().mockImplementation((question) => {
//       if (question.type === 'list') return Promise.resolve({ selectChoice });
...
//     const log = jest.fn().mockImplementation((res)=>console.log(res));
...
//       expect(log.mock.calls[0][0]).toEqual(selectChoice);
//     });
...
//         if (mockPrompt.mock.calls.length === 1) return Promise.resolve({ selectChoice: 'next' });
//         return Promise.resolve({ selectChoice: {} });
//       });
...
//       const mockPrompt = jest.fn();
//       mockPrompt.mockImplementation(() => {
//         if (mockPrompt.mock.calls.length === 1) return Promise.resolve({ selectChoice: 'prev' });
//         return Promise.resolve({ selectChoice: {} });
//       });
...
//     const inquirer = {
//       prompt: jest.fn().mockResolvedValueOnce({ selectChoice: 'back' }),
//       Separator: inquirerModule.Separator,
//     };
