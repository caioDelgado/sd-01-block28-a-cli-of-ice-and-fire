...
async function repeatList(goBack, dependencies, url) {
...
async function listHouses(goBack, dependencies, links, choices, url) {
  ...
  return inquirer.prompt({
    ...
    .then(({ house }) => {
      ...
      return repeatList(goBack, dependencies, url);
    });
}

async function chooseOption(choices, link, dependencies, goBack, url) {
  const { inquirer } = dependencies;
...
async function run(goBack, dependencies, url) {
  ...
  return request.then(({ body, headers: { link } }) => {
    ...
    return chooseOption(choices, link, dependencies, goBack, url);
  });
}
