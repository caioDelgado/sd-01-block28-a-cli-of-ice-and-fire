...
function returnChoices(choices, link, resolve, goBack, dependencies, url) {
  const { log, inquirer } = dependencies;
...
async function run(goBack, dependencies = {}, url) {
  return new Promise((resolve) => {
    ...
    request.then(({ body, headers: { link } }) => {
      ...
      returnChoices(choices, link, resolve, goBack, dependencies, url);
    });
  });
}
