...
function getSuperagentMock(response) {
  ...
  superagent.get.mockImplementation(() => response);
...
describe('List action', () => {
  describe('presented options', () => {
    ...
    const superagent = getSuperagentMock(housesFixture.responses.hasNext);

    beforeAll(async () => {
      const inquirer = {
        prompt: jest.fn().mockResolvedValue({ house: {} }),
    ...
    test('calls API to get results', () => {
      expect(superagent.get).toBeCalledWith('https://www.anapioficeandfire.com/api/houses/?page=1&pageSize=10');
    });

    test('presents all returned results as options', () => {
      expect(choices).toEqual(expect.arrayContaining(housesFixture.names.hasNext));
    });

    test('presents a "back" option', () => {
      expect(choices).toContain('Voltar para o menu das casas');
    });
  });

  describe('details', () => {
    const superagent = getSuperagentMock(housesFixture.responses.hasNext);
    const house = housesFixture.responses.hasNext.body[0];
    const mockPrompt = jest.fn().mockImplementation((question) => {
      if (question.type === 'list') return Promise.resolve({ house });
    ...
    test('display details of selected houses', async () => {

      expect(log.mock.calls[0][0]).toEqual(house);
    });

    test('offers to go back to houses list after showing details', () => {
  ...
  describe('pagination', () => {
    describe('next', () => {
      ...
      mockPrompt.mockImplementation(() => {
        if (mockPrompt.mock.calls.length === 1) return Promise.resolve({ house: 'next' });
      ...
      test('requests next page from API when "next" option is selected', () => {
        expect(superagent.get).toBeCalledWith('https://www.anapioficeandfire.com/api/houses?page=2&pageSize=10');
      });
    });

    describe('previous', () => {
      const superagent = getSuperagentMock(housesFixture.responses.hasPrevious);
      const mockPrompt = jest.fn();
      mockPrompt.mockImplementation(() => {
        if (mockPrompt.mock.calls.length === 1) return Promise.resolve({ house: 'prev' });
        return Promise.resolve({ house: {} });
      });
      ...
      test('requests previous page from API when "previous" option is selected', () => {
        expect(superagent.get).toBeCalledWith('https://www.anapioficeandfire.com/api/houses?page=443&pageSize=10');
      });
    });
  });

  describe('actions', () => {
    const superagent = getSuperagentMock(housesFixture.responses.hasBoth);
    const inquirer = {
      prompt: jest.fn().mockResolvedValueOnce({ house: 'back' }),
      Separator: inquirerModule.Separator,
    };
