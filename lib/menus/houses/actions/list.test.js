...
function getSuperagentMock(response) {
  const superagent = {
    get: jest.fn(),
    query: jest.fn(),
    then: jest.fn().mockImplementation(() => Promise.resolve(response)),
  };
...
describe('List action', () => {
  describe('presented options', () => {
    ...
    test('calls API to get results', () => {
      expect(superagent.get).toBeCalledWith('https://www.anapioficeandfire.com/api/houses');
    ...
    test('presents a "back" option', () => {
      expect(choices).toContain('Voltar para o menu de casas');
    });
  });
  ...
  describe('details', () => {
    ...
    test('offers to go back to house list after showing details', () => {
  ...
  describe('pagination', () => {
    ...
    describe('previous', () => {
      ...
      test('requests previous page from API when "previous" option is selected', () => {
        expect(superagent.get).toBeCalledWith('https://www.anapioficeandfire.com/api/houses?page=45&pageSize=10');
        expect(superagent.query).toBeCalledTimes(1);
      });
    });
  });
