...
async function run(goBack, dependencies = {}, url) {
  return new Promise((resolve, reject) => {
    ...
    if (!url) request.query({ page: 1, pageSize: 10 }); // elementos de consulta vem com esse valor na primeira solicitação;

    request.end((err, { body, headers: { link } }) => { // responsável por receber o retorno da solicitação
      // dependendo da string de consulta  o headers.link sofre mudanças;
      ...
      const choices = body.map((character) => {
        ...
        return {
          name: name || aliases[0], // se o filme tiver nome, pega o nome, senão pega o apelido que esta em um array;
